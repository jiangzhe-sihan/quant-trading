...
prop = [
    ...
    ('std-v', lambda x: x.ema(5, 'volume') / x.ema(4, x.std(3, x.get_series('volume')))[x.date])
]
...
def func(self):
    ...
    for k, v in self.market.tell.items():
        ...
        if v.code not in self.static:
            ...
            output = v.ema(5, vol) / v.ema(3, v.std(5, vol))
            self.static[v.code] = ((v.max(pwm1, pwm2) > 1.1) | (vrsi > 36)) & (stick > .65) & (bl < 1.9) & (c >= o) & (inc > 1.03) & (~zt | (hs < 5.9)) & (zf > .15) & ~v.between(fl, .33, .55) & ~v.between(output, 3, 7)
        if self.static[v.code][v.date] and v.hs > 5 and len(self.warehouse) < 5:
            self.li_buy.add(k)
