...
def func(self):
    ...
    for k, v in self.market.tell.items():
        ...
        if v.code not in self.static:
            ...
            hc = v.between(inc, .97, .98)
            zt = c >= v.zt_price(v.ref(1, c), .06)
            cci = v.get_series('cci')
            self.static[v.code] = buy_all & (v.count(buy2, n) / n < .2) & v.ref(1, zt) & (v.ref(1, v.exist(hc, 3))) & (cci > v.ref(1, cci))
        if self.static[v.code][v.date] and v.amount > 50000000 and v.close > 20:
            self.li_buy.add(k)
