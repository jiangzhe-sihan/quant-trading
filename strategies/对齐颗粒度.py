...
def func(self):
    ...
    for k, v in self.market.tell.items():
        ...
        if v.code not in self.static:
            ...
            buy_all = buy1 & ~buy2
            hc = v.between(inc, .93, .98)
            zt = c >= v.zt_price(v.ref(1, c), .07)
            cci = v.get_series('cci')
            self.static[v.code] = buy_all & (v.count(buy2, n) / n < .2) & (v.ref(1, zt)) & (v.ref(1, v.exist(hc, 4))) & (cci > v.ref(1, cci))
        if self.static[v.code][v.date] and v.market_value > 3000000000:
            self.li_buy.add(k)
